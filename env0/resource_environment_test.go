...
func TestUnitEnvironmentResource(t *testing.T) {
	...
	testWaitFor := func() {
		createEnvironmentResourceConfigWithWaitFor := func(environment client.Environment, waitFor string) string {
			return resourceConfigCreate(resourceType, resourceName, map[string]interface{}{
				"name":          environment.Name,
				"project_id":    environment.ProjectId,
				"template_id":   environment.LatestDeploymentLog.BlueprintId,
		...
		updatedEnvironmentForRedeploy := client.Environment{
			Id:            environment.Id,
			Name:          environment.Name,
			ProjectId:     environment.ProjectId,
			WorkspaceName: environment.WorkspaceName,
			LatestDeploymentLog: client.DeploymentLog{
				Id:                deploymentLogId,
				BlueprintId:       templateId,
				BlueprintRevision: "new-revision-so-redeploy",
			},
		}

		inProgressStatuses := []string{"IN_PROGRESS", "QUEUED", "WAITING_FOR_USER"}
		for _, inProgressStatus := range inProgressStatuses {
			t.Run(fmt.Sprintf("wait_for=FULLY_DEPLOYED, deployment is %s", inProgressStatus), func(t *testing.T) {
				inProgressDeploymentLog := client.DeploymentLog{
					Id:          deploymentLogId,
					BlueprintId: templateId,
					Status:      inProgressStatus,
				}

				doneDeploymentLog := client.DeploymentLog{
					Id:          deploymentLogId,
					BlueprintId: templateId,
					Status:      "SUCCESS",
				}
				...
				testCase := resource.TestCase{
					Steps: []resource.TestStep{
						{
							Config: createEnvironmentResourceConfigWithWaitFor(environment, "FULLY_DEPLOYED"),
							Check: resource.ComposeAggregateTestCheckFunc(
								resource.TestCheckResourceAttr(accessor, "id", environment.Id),
								resource.TestCheckResourceAttr(accessor, "name", environment.Name),
								resource.TestCheckResourceAttr(accessor, "project_id", environment.ProjectId),
						...
						{
							Config: createEnvironmentResourceConfigWithWaitFor(updatedEnvironmentForRedeploy, "FULLY_DEPLOYED"),
							Check: resource.ComposeAggregateTestCheckFunc(
								resource.TestCheckResourceAttr(accessor, "id", updatedEnvironmentForRedeploy.Id),
								resource.TestCheckResourceAttr(accessor, "name", updatedEnvironmentForRedeploy.Name),
								resource.TestCheckResourceAttr(accessor, "project_id", updatedEnvironmentForRedeploy.ProjectId),
								resource.TestCheckResourceAttr(accessor, "template_id", templateId),
								resource.TestCheckResourceAttr(accessor, "workspace", updatedEnvironmentForRedeploy.WorkspaceName),
							),
						},
					},
				}
				...
				runUnitTest(t, testCase, func(mock *client.MockApiClientInterface) {
					...
					gomock.InOrder(
						mock.EXPECT().Deployment(gomock.Any()).Times(2).Return(inProgressDeploymentLog, nil), // CREATE - 2 times the deployment is still in progress
						mock.EXPECT().Deployment(gomock.Any()).Times(1).Return(doneDeploymentLog, nil),       // CREATE - 1 time check it's done
						mock.EXPECT().Deployment(gomock.Any()).Times(2).Return(inProgressDeploymentLog, nil), // REDEPLOY - 2 times the deployment is still in progress
						mock.EXPECT().Deployment(gomock.Any()).Times(1).Return(doneDeploymentLog, nil),       // REDEPLOY - 1 time check it's done
						mock.EXPECT().Deployment(gomock.Any()).Times(2).Return(inProgressDeploymentLog, nil), // DESTROY - 2 times the deployment is still in progress
						mock.EXPECT().Deployment(gomock.Any()).Times(1).Return(doneDeploymentLog, nil),       // DESTROY - 1 time check it's done
					)
					mock.EXPECT().EnvironmentDeploy(environment.Id, gomock.Any()).Times(1).Return(client.EnvironmentDeployResponse{
						Id: deploymentLogId,
					}, nil)
					...
					gomock.InOrder(
						mock.EXPECT().Environment(gomock.Any()).Times(2).Return(environment, nil),                   // 1 after create, 1 before update
						mock.EXPECT().Environment(gomock.Any()).Times(1).Return(updatedEnvironmentForRedeploy, nil), // 1 after update
					)
					mock.EXPECT().EnvironmentDestroy(environment.Id).Times(1)
				})
			})
		...
		doneStatuses := []string{"SUCCESS", "SKIPPED"}
		for _, doneStatus := range doneStatuses {
			t.Run(fmt.Sprintf("wait_for=FULLY_DEPLOYED, deployment is %s", doneStatus), func(t *testing.T) {
				doneDeploymentLog := client.DeploymentLog{
					Id:          deploymentLogId,
					BlueprintId: templateId,
					Status:      doneStatus,
				...
				runUnitTest(t, testCase, func(mock *client.MockApiClientInterface) {
					...
					mock.EXPECT().Deployment(gomock.Any()).Times(3).Return(doneDeploymentLog, nil) // Check deployment is done, 1 for create, 1 for update, 1 for destroy
					gomock.InOrder(
						mock.EXPECT().Environment(gomock.Any()).Times(2).Return(environment, nil),                   // 1 after create, 1 before update
						mock.EXPECT().Environment(gomock.Any()).Times(1).Return(updatedEnvironmentForRedeploy, nil), // 1 after update
					)
		...
		failedStatuses := []string{"TIMEOUT", "FAILURE", "CANCELLED", "INTERNAL_FAILURE", "ABORTING", "ABORTED"}
		for _, failedStatus := range failedStatuses {
			t.Run(fmt.Sprintf("wait_for=FULLY_DEPLOYED, deployment is %s", failedStatus), func(t *testing.T) {
				failedDeploymentLog := client.DeploymentLog{
					Id:          deploymentLogId,
					BlueprintId: templateId,
					Status:      failedStatus,
				}
				...
				testCase := resource.TestCase{
					Steps: []resource.TestStep{
						{
							Config:      createEnvironmentResourceConfigWithWaitFor(environment, "FULLY_DEPLOYED"),
							ExpectError: regexp.MustCompile(fmt.Sprintf("environment deployment reached failure status: %s", failedStatus)),
						},
					},
				}
				...
				runUnitTest(t, testCase, func(mock *client.MockApiClientInterface) {
					...
					mock.EXPECT().Deployment(gomock.Any()).Times(2).Return(failedDeploymentLog, nil) // Check deployment is done. 1 for create, 1 for destroy
					mock.EXPECT().EnvironmentDestroy(environment.Id).Times(1)
				})
			})
		}

		t.Run("wait_for=ACK, should not wait for any deployment to finish", func(t *testing.T) {
			testCase := resource.TestCase{
				Steps: []resource.TestStep{
					{
						Config: createEnvironmentResourceConfigWithWaitFor(environment, "ACK"),
						Check: resource.ComposeAggregateTestCheckFunc(
							resource.TestCheckResourceAttr(accessor, "id", environment.Id),
							resource.TestCheckResourceAttr(accessor, "name", environment.Name),
							resource.TestCheckResourceAttr(accessor, "project_id", environment.ProjectId),
					...
					{
						Config: createEnvironmentResourceConfigWithWaitFor(updatedEnvironmentForRedeploy, "ACK"),
						Check: resource.ComposeAggregateTestCheckFunc(
							resource.TestCheckResourceAttr(accessor, "id", updatedEnvironmentForRedeploy.Id),
							resource.TestCheckResourceAttr(accessor, "name", updatedEnvironmentForRedeploy.Name),
							resource.TestCheckResourceAttr(accessor, "project_id", updatedEnvironmentForRedeploy.ProjectId),
