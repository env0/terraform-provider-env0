...
var _ = Describe("Organization", func() {
	...
	Describe("OrganizationCreate", func() {
		createPayload := OrganizationCreatePayload{
			Name:        "test-org",
		...
		Describe("Success", func() {
			BeforeEach(func() {
				httpCall = mockHttpClient.EXPECT().
					Post("/organizations", createPayload, gomock.Any()).
					Do(func(path string, request interface{}, response *Organization) {
						*response = mockOrganization
					})

				createdOrganization, err = apiClient.OrganizationCreate(createPayload)
			})
			...
			It("Should return organization received from API", func() {
				Expect(*createdOrganization).To(Equal(mockOrganization))
			})
		})
		...
		Describe("Failure", func() {
			It("Should return error when API call fails", func() {
				...
				Expect(createdOrganization).To(BeNil())
			})
		})
	})

	Describe("OrganizationUpdate", func() {
		updatePayload := OrganizationUpdatePayload{
			Name:        "updated-org",
			Description: "updated description",
			PhotoUrl:    "https://test.com/updated-photo.jpg",
		}
		...
		Describe("Success", func() {
			BeforeEach(func() {
				httpCall = mockHttpClient.EXPECT().
					Put("/organizations/"+organizationId, updatePayload, gomock.Any()).
					Do(func(path string, request interface{}, response *Organization) {
						*response = mockOrganization
					})

				updatedOrganization, err = apiClient.OrganizationUpdate(organizationId, updatePayload)
			})
			...
			It("Should return organization received from API", func() {
				Expect(*updatedOrganization).To(Equal(mockOrganization))
			})
		})

		Describe("Failure", func() {
			It("Should return error when API call fails", func() {
				expectedError := errors.New("failed to update organization")
				mockHttpClient.EXPECT().
					Put("/organizations/"+organizationId, updatePayload, gomock.Any()).
					Return(expectedError)
				...
				updatedOrganization, err = apiClient.OrganizationUpdate(organizationId, updatePayload)
				Expect(err).To(Equal(expectedError))
				Expect(updatedOrganization).To(BeNil())
			})
		})
	})

	Describe("OrganizationById", func() {
		var organization *Organization
		var err error
		...
		Describe("Success", func() {
			BeforeEach(func() {
				httpCall = mockHttpClient.EXPECT().
					Get("/organizations/"+organizationId, nil, gomock.Any()).
					Do(func(path string, request interface{}, response *Organization) {
						*response = mockOrganization
					})

				organization, err = apiClient.OrganizationById(organizationId)
			})

			It("Should send GET request with expected organization id", func() {
				httpCall.Times(1)
			})
			...
			It("Should return organization received from API", func() {
				Expect(*organization).To(Equal(mockOrganization))
			})
		})

		Describe("Failure", func() {
			It("Should return error when API call fails", func() {
				expectedError := errors.New("failed to get organization")
				mockHttpClient.EXPECT().
					Get("/organizations/"+organizationId, nil, gomock.Any()).
					Return(expectedError)
